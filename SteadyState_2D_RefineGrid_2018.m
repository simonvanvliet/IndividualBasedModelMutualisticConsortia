function [output,muAsFunctionOfGridSize,finalGridScaling,varargout]=SteadyState_2D_RefineGrid_2018(settings,input)
...
output=SteadyState_2D_SOR_Solver_2018(settings,input);

muAsFunctionOfGridSize.(['gridScale_',num2str(settings.gridScaling)])=output.mu;
if TalkToMe; fprintf('SteadyState_2D_RefineGrid_2018-> Increasing gridScaling to %i, check if mu has converged\n',2*settings.gridScaling); end
...
while ~muConverged % refine grid
    ...
    if muError>settings.muTolerance
        if TalkToMe; fprintf('SteadyState_2D_RefineGrid_2018-> Increasing gridScaling to %i, max abs error mu=%.2g\n',2*settings.gridScaling,muError); end
        if settings.gridScaling>=settings.maxGridScaling
            muConverged=1;
            fprintf('!!! SteadyState_2D_RefineGrid_2018-> mu has not converged. ru=%.2g, rl=%.2g\n',settings.ru,settings.rl);
        end
    else
        muConverged=1;
        if TalkToMe; fprintf('SteadyState_2D_RefineGrid_2018-> mu has converged. GridScale used=%i, max abs error mu=%.2g\n',settings.gridScaling,muError);end
    end
end
